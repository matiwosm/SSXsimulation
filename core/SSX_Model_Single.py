...
##########################################################################################################################################
            if(r <= 0.8 - lambda_rho_r):
               ...
               fullGrid[i][j][k] = (fullGrid[i][j][k] + rho_min)/2 + (fullGrid[i][j][k] - rho_min)/2*np.sin((1 - r)*np.pi/(2*lambda_rho_r))
            else:
               fullGrid[i][j][k] = rho_min
...
field_writes = solver.evaluator.add_file_handler('fields_one_update_v3', max_writes = field_max_writes, sim_dt = output_cadence, mode = 'overwrite')
...
parameter_writes = solver.evaluator.add_file_handler('parameters_one_update_v3', max_writes = parameter_max_writes, sim_dt = output_cadence, mode = 'overwrite')
...
load_writes = solver.evaluator.add_file_handler('load_data_one_update_v3', max_writes = load_max_writes, sim_dt = output_cadence, mode = 'overwrite')
...
char_time = cfl_params.getfloat('char_time') # this should be set to a characteristic time in the problem (the alfven crossing time of the tube, for example)
