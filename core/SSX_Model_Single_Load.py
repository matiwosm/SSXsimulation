...
args = docopt(__doc__)
load_file = pathlib.Path(sys.argv[-1])
...
########################################################################################################
write, dt = solver.load_state(load_file)

"""
The line below is not needed the inital time step will be obtained from the solver.load()
# Initial timestep
dt = float(args['<initStep>'])
"""
# Getting solver states
Ax = solver.state['Ax']
Ay = solver.state['Ay']
Az = solver.state['Az']
...
output_cadence = 0.10 # This is in simulation time units
...
field_writes = solver.evaluator.add_file_handler('fields_one_update_v2', max_writes=50, sim_dt = output_cadence, mode='append')
...
load_writes = solver.evaluator.add_file_handler('load_data_one_update_v2', max_writes=50, sim_dt = output_cadence, mode='append')
